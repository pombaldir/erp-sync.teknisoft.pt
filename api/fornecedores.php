...
$queryCliente = $database->select("Tbl_Fornecedores", ["Id"], ["intCodigo" => $intCodigo]);	      
...
if(is_array($queryCliente) && sizeof($queryCliente)>0){
    ...
    $database->insert("Tbl_Fornecedores", [
        "intCodigo" => $intCodigo,
...
$numArtigo=$_GET['num'];
 $queryCliente = $database->select("Tbl_Fornecedores", [         
  "Id",
  "intCodigo",
  "strNome",
  "strNumContrib",
...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
if($accao=="contactos" && isset($_GET['num']) && $_SERVER['REQUEST_METHOD'] === 'GET'){
    $numEntidade=$_GET['num'];
    $tpcontacto=$_GET['strCodTpContacto'];

    $arrPARAMS['intTipoEntidade']=2;

    $arrPARAMS['strChaveEntidade']="".$numEntidade."";
    
    if($tpcontacto!="") { $arrPARAMS['strCodTpContacto']=$tpcontacto; }

    if ( isset($_GET['searchField']) && $_GET['searchField'] != "" && isset($_GET['q']) && $_GET['q'] != "")
    {
        $arrPARAMS[''.$_GET['searchField'].'']="".$_GET['q']."";
    }	 
   // die (json_encode($_GET));
    $output = $database->select("Tbl_Contactos", [         
        "intCodContacto",
        "strNome",
        "strCargo",
        "intCodEntRelacionada",
        "strNomeEndereco", 
        "strMorada1",
        "strMorada2",
        "strLocalidade",
        "strCodPostal",
        "strTelEmpresa",
        "strTelemovel", "strEmail1", "strEmail2", "strWebsite", 
        "dtmNascimento", "intNumLinha", "imgFoto", "strCodTpContacto", "strTelParticular", "strObservacoes", "strLoginAbertura", "strLoginAlteracao", "strAbrevZona", "strAbrevSubzona", "strChaveEntidade", 
         "strNumContrib", "bitInactivo","intTipoEntidade",
        "dtmAbertura",   
        "dtmAlteracao"   
       ], $arrPARAMS); 
...
if($accao=="list"){
    ...
    $sIndexColumn = "Id"; 
    /* DB table to use */ 
    $sTable = "Tbl_Fornecedores"; 
    /*
    * Columns
    */ 
	$aColumns = array("Id","intCodigo", "strNome", "strNumContrib", "strTelefone",  "strEmail", "strMorada_lin1", "strMorada_lin2", "strLocalidade","strPostal","strAbrevSubZona","intCodCatEntidade","strCodCondPag","strObs","dtmAbertura","dtmAlteracao");    
