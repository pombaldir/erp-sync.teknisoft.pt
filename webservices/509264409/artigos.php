...
if(isset($_GET['act_g']) && $act_get=="sku"){
	...
	$rResult = mssql_query("SELECT QuantStock from Gce_stk_real where strCodArtigo='$codartigo'") or die( print_r( sqlsrv_errors(), true));   

	$l = mssql_fetch_array($rResult);
	
	$stock=number_format($l['QuantStock'],2,".",""); 
	$found=mssql_num_rows($rResult);		   
	...
	$rResult = mssql_query("SELECT Tbl_Gce_ArtigosIdiomas.strDescricao, Tbl_Gce_ArtigosIdiomas.strDescricaoCompl, Tbl_Gce_ArtigosIdiomas.strExcerto, Tbl_Gce_ArtigosIdiomas.strCodIdioma, Tbl_Gce_ArtigosIdiomas.strCodArtigo
