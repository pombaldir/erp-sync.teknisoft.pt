...
if((isset($_GET['auth_userid']) && $_GET['auth_userid']=="SmA13z?xU(hxR4e") || (isset($_POST['auth_userid']) && $_POST['auth_userid']=="SmA13z?xU(hxR4e")) {
...
if(isset($act_get) && $act_get=="sku" && !isset($_POST['act_p'])){  
		
$codartigo=$_GET['num']; 

	$query = mssql_query("SELECT QuantStock from Gce_stk_real where strCodArtigo='$codartigo'") or die("Erro");
	$l = mssql_fetch_array($query);
	$stock=number_format($l['QuantStock']);
	$found=mssql_num_rows($query);	
	
	
	
	if(isset($_GET['upd']) && $_GET['upd']=="1"){
	if(($_GET['is_in_stock']=="true" && $stock<=0) || ($_GET['is_in_stock']=="false" && $stock>0)){
	
	$_SESSION['empresaID']=4;	
	include("/home/teknisoftcp/erp-sync.teknisoft.pt/backoffice/include/psl-config.php");
	include("/home/teknisoftcp/erp-sync.teknisoft.pt/backoffice/include/functions.woocommerce.php");
		
		$data = [
			'stock_quantity' => ''.$stock.'',
			'in_stock' => $_GET['is_in_stock'],
		];
		
		edit_produto_variation($_GET['id'],$_GET['idsku'],$data);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	if(isset($_POST['act_p']) && $act_pst=="resetArtigos"){
	mssql_query("UPDATE Tbl_Gce_Artigos set bitPortalWeb=0") or $html_msg=mssql_get_last_message();
	$output = array("success" => 1);	
	}	
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	if(isset($_POST['act_p']) && $act_pst=="updtArtigos"){
	$sku=$_POST['sku'];	
	mssql_query("UPDATE Tbl_Gce_Artigos set bitPortalWeb=1 WHERE strCodigo='$sku'") or $html_msg=mssql_get_last_message();
	
	$html_msg="";		
	$query = "SELECT Id from Tbl_Gce_Artigos where strCodigo='$sku'";
	$result = mssql_query($query) or $html_msg=mssql_get_last_message();
	$post = mssql_fetch_array($result); 
	$Id=$post['Id'];  	
			 
	$output = array("success" => 1,"message"=> "$html_msg","Id"=> "$Id");	
	}	
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
