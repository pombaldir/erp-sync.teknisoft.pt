...
//$meta_data=unserialize($dados['meta_data']);
...
$rDocExiste = $database->select("Mov_Encomenda_Cab", ['Id'], ["strNumExterno" => $id,"strAbrevTpDoc" => $EncstrAbrevTpDoc,"strCodSeccao" => $strCodSeccao]);
//erpLog($database->log());   
...
if(sizeof($rDocExiste)==0){ 
	erpLog("Doc não existe"); 
#################################################################	ADICIONA CLIENTE  ######################################################		
	if($bitConsumidorFinal==0){

	$strObs="Cliente $nif criado via Loja Online";
	erpLog($strObs); 
	$rq2=$database->select("Tbl_Clientes", ['Id'], ["strNumContrib[!]" => NULL,"strNumContrib[!]" => "CF","strNumContrib" => $nif]);
	//$rq3=$database->count("Tbl_Clientes", ['intCodigo' => $nCliente]);
	//erpLog($database->log());   

	if(strlen($nif)>=9 && sizeof($rq2)>0){
	...
	$ListCliente=$database->select("Tbl_Clientes", ["intCodigo"], ["intCodigo[<]"=> $ClientesLower, "ORDER" => ["Tbl_Clientes.Id" => "DESC"],"LIMIT" => 1]);
	$nCliente=$ListCliente[0]['intCodigo']+1; // Nº DE CLIENTE
	} 
	if(isset($tpnumeracao) && $tpnumeracao=="tlm"  && $nif!=""){
	...
	$contacto_existe=$database->select("Tbl_Contactos", ['Id'], ["strChaveEntidade" => $nCliente,"strCodTpContacto" => "$depFinTpContacto","intTipoEntidade"=>1]);
	if(sizeof($contacto_existe)==0){
...
$quantidTotal=0;
	...
	$database->insert("Mov_Venda_Cab", [
		...
		"strCVDNumContrib" => "$nif",
		"strCVDNome" => "$nomeClie",
		"strCVDMorada" => "$b_address_1 $b_address_2",
		"strCVDCodPostal" => "$b_postcode",
		"strCVDTelefone" => "$b_phone",
		"strCVDLocalidade" => "$b_city",
        "strCVDEmail" => "$b_email",
	...
	$count = $database->select("Tbl_Gce_Artigos", ['Id'], [
	...
	$database->insert("Mov_Venda_Lin", [
		...
		//"fltPendingQuantity" => $vartigo['quantity'],		// RESERVA STOCK
