...
if(isset($_GET['act_g']) && $act_get=="list" && ($_SERVER['REQUEST_METHOD'] === 'GET')){
    ...
    * Columns
    */ 
	$aColumns = array('Id','strCodigo', 'strDescricao', 'bitPortalWeb', 'strCodCategoria');      
    /* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */     
    /*
     * Ordering
    */
    ...
    $sWhere = "WHERE Tbl_Gce_Referencias.bitInactivo=0 ";
	...
	if(isset($_GET['Familia']) && $_GET['Familia']!=""){
		...
		$sWhere .= "Tbl_Gce_ReferenciasFamilias.strCodFamilia='".$_GET['Familia']."'";  
	}
	
	if(isset($_GET['Marca']) && $_GET['Marca']!=""){
		if($sWhere==""){ $sWhere .= " WHERE "; } else { $sWhere .= " AND "; } 
		$sWhere .= "TTbl_Gce_ReferenciasFamilias.strCodFamilia='".$_GET['Marca']."'";  
	}
	
	if(isset($_GET['bitDispWeb']) && $_GET['bitDispWeb']!="" && $_GET['bitDispWeb']!="0"){
		if($sWhere==""){ $sWhere .= " WHERE "; } else { $sWhere .= " AND "; } 
		$sWhere .= "Tbl_Gce_Referencias.bitPortalWeb='".$_GET['bitDispWeb']."'";  
	}
	
	
	$sQuery = $database->query("SELECT * FROM (SELECT Tbl_Gce_Referencias.strCodigo,Tbl_Gce_Referencias.strDescricao,Tbl_Gce_Referencias.bitPortalWeb,
	Tbl_Gce_Referencias.Id,strCodFamilia,
	ROW_NUMBER() OVER (PARTITION BY Tbl_Gce_Referencias.Id ORDER BY Tbl_Gce_Referencias.Id ASC) AS RowNumber
	FROM Tbl_Gce_Referencias 
	LEFT JOIN [Tbl_Gce_ReferenciasFamilias] ON [Tbl_Gce_Referencias].[strCodigo] = [Tbl_Gce_ReferenciasFamilias].[strCodReferencia] 
	...
	$data = $database->query("SELECT COUNT(strCodigo) as total FROM (SELECT [Tbl_Gce_Referencias].[strCodigo],
	ROW_NUMBER() OVER (PARTITION BY Tbl_Gce_Referencias.Id ORDER BY Tbl_Gce_Referencias.Id DESC) AS RowNumber FROM [Tbl_Gce_Referencias] 
	LEFT JOIN [Tbl_Gce_ArtigosFamilias] ON [Tbl_Gce_Referencias].[strCodigo] = [Tbl_Gce_ArtigosFamilias].[strCodArtigo] 
