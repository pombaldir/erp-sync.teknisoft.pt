...
//@unlink('my-errors.log');  
//error_log(serialize($_POST['dados']), 3, "debug-log.log");       
//die();   
...
//$dados=base64_decode(unserialize($_POST['dados'])); 
$dados=unserialize(base64_decode($_POST['dados']));  
...
//error_log("Encomenda $id - ".base64_decode($_POST['dados'])." \n", 3, "debug-log.log");      
//die();       
...
$b_phone=stripslashes($billing['phone_mobile']);
...
$nif=stripslashes($customer['siret']);      
...
$linhas[$kartigo]=array("quantity"=>$vartigo['product_quantity'],"product_sku"=>$vartigo['product_reference'],"total"=>$vartigo['total_price_tax_incl'],"name"=>$vartigo['product_name'],"product_price"=>$vartigo['unit_price_tax_incl']);
}    
...
if($b_company!="" && strlen($b_company)>3){ $nomeClie=$b_company; }

$nif=preg_replace('/\s+/', '', $nif);
$nif=substr($nif, -9);
//if($nif=="" || strlen($nif)<9){ $bitConsumidorFinal=1; } else { $bitConsumidorFinal=0; }
$bitConsumidorFinal=0;


//error_log("Encomenda $id - NIF=$nif \n", 3, "debug-log.log"); 
//die(); 
$rDocExiste = $database->count("Mov_Encomenda_Cab", ["strNumExterno" => "$id","strAbrevTpDoc" => $EncstrAbrevTpDoc,"strCodSeccao" => $strCodSeccao]);
...
//error_log("Encomenda $id - ".json_encode($database->log())."\nEncomenda $id - DOC existe: $rDocExiste\n", 3, "debug-log.log");        
 
if($rDocExiste==0){ 
	//error_log("DOC Não existe (CF=$bitConsumidorFinal) \n", 3, "debug-log-3.log");   
#################################################################	ADICIONA CLIENTE  ######################################################		
	if($bitConsumidorFinal==0){

	if($nif=="" || strlen($nif)<9){ $bitConsumidorFinal=1; } else { $bitConsumidorFinal=0; }
	//if($nif=="" || strlen($nif)<9){ $bitConsumidorFinal=1; } else { $bitConsumidorFinal=0; }
	//error_log("Encomenda $id - Não é consumidor final \n", 3, "debug-log.log"); 
	...
	$rq2=$database->count("Tbl_Clientes", ["strNumContrib[!]" => "CF","intCodigo" => "$b_phone"]); 

	//error_log("Encomenda $id - cliente existe? $rq2\n", 3, "debug-log.log");        


	if((strlen($nif)>=9 || strlen($b_phone)>=9) && $rq2>0){
	$qCliente=$database->select('Tbl_Clientes', ['intCodigo'],["strNumContrib[!]" => "CF","intCodigo" => "$b_phone"]); 

	$nCliente=$qCliente[0]['intCodigo'];
	$htmlmsg="Já existe cliente com NIF $nif (Cód: $nCliente)"; 

	//error_log("Encomenda $id - $htmlmsg\n", 3, "debug-log.log");  
	...
	//error_log("Encomenda $id - Temos de criar cliente... $tpnumeracao\n", 3, "debug-log.log");  
		
	if(!isset($tpnumeracao) || $tpnumeracao=="sequencial" && $nif!=""){
	...
	//error_log("Encomenda $id - Criar cliente $nCliente\n", 3, "debug-log.log");   	
	}
	...
	//error_log("Encomenda $id - Cliente $nCliente, CF: $bitConsumidorFinal, NIF: $nif, Telefone: $b_phone", 3, "debug-log.log");  
	//error_log("Encomenda $id - ".json_encode($database->log())."\n", 3, "debug-log.log");  
...
## Constantes 
	...
	$strHora=date('H:i:s'); 
	$mensagemExtra="Criado via encomendas web: #$id \n$customer_note\n";
## /Constantes 
...
$quantidTotal=0;
	...
	$database->insert("Mov_Encomenda_Lin", [
		...
		"fltPrecoUnitario" => $vartigo['product_price'], 	// COM IVA   
		"fltValorLiquido" => $vartigo['total'],
		"fltTaxaIVA" => $taxaIVA,
		"intCodTaxaIVA" => $CodTaxaIva,
		"fltValorMercadoriaCIVA" => $vartigo['total'],
		"fltValorMercadoriaSIVA" => $fltValorMercadoriaSIVA,
	...
	$database->insert("Mov_Encomenda_Lin", [
		...
		"strDescArtigo" => "Portes de envio",
		"fltQuantidade" => 1,
		"fltQuantidadePend" => 1,
		"fltPrecoUnitario" => $shipping_total, // SEM IVA 
