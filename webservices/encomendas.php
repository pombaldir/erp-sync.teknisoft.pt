...
//mail('nsantos@pombaldir.com', 'LOG '.$_POST['store'].' #'.$order_id.'', $message);
...
$shipping_total=stripslashes($total_wrapping_tax_incl); 
...
$b_address_1=titleCase($b_address);
$b_postcode=str_replace("/","-",$b_zipcode);
$b_company=titleCase($company);
$b_city=titleCase($b_city);
$b_email=str_replace(" ","",$email);  
$id=$order_id;
$nCliente=$b_phone;  

$s_company="";
$s_address_1=titleCase($s_address);
$s_postcode=str_replace("/","-",$s_zipcode);
$nif=$fields['42'];
...
foreach($products as $k=>$p){
	$codartigo=$p['product_code'];
	if($tokenAPI=="24PbJ7xKCM3zutcC"){ 		// MOD MALAS VITESS
		if (strpos($codartigo, 'x') === 0 || strpos($codartigo, 'X') === 0) {
		$codartigo=substr($codartigo, 1);
		}
	}
$linhas[]=array('name'=>$p['product'],'product_sku'=>$codartigo,'product_price'=>$p['price'],'quantity'=>$p['amount'],'total'=>$p['base_price']);
} 


$total_tax=0; 
foreach($taxes as $kimp=>$imposto){
	$total_tax=$total_tax+$imposto['tax_subtotal'];
}
  

//die(error_log(serialize($linhas), 3, "/home/erpsinc/public_html/webservices/my-errors.log"));
...
$nomeClie=titleCase($b_firstname)." ".titleCase($b_lastname);
if($b_company!="" && strlen($b_company)>3){
	$nomeClie=titleCase($b_company); 
}   
...
#################################################################	ADICIONA CLIENTE  ######################################################		
	...
	$nCliente=preg_replace('/\D/', '', $b_phone);
	$nCliente=substr($nCliente, -9); 
	$b_phone=$nCliente;   	
	}

	//die(json_encode($nCliente));  	 


	//error_log("$nCliente", 3, "/home/erpsinc/public_html/webservices/my-errors.log");
...
$mensagemHtml.="<b>$htmlmsg</b>".$loghtmlBD."<hr><b>id:</b> $id <b>status:</b> ".$status." <b>Nome:</b>".$billing['first_name']." ".$b_lastname." <b>Morada:</b>".$b_address_1." ".$b_address_2."<hr><b>Linhas:</b> ".json_encode($linhas)."";  
