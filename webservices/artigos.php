...
header('Access-Control-Allow-Methods: GET, POST, PUT');   if(isset($_POST['act_p']) && $_POST['act_p']=="editFoto") {} else {
...
if(isset($_GET['act_g']) && $act_get=="sku" && ($_SERVER['REQUEST_METHOD'] === 'GET')){
	...
	*/
	...
	$urlws="https://erpsinc.pt/webservices/api.php?auth_userid=".$tokenAPI."&act_g=getEmp";
	$json=file_get_contents("$urlws", false, stream_context_create($arrContextOptions));
	$obj = json_decode($json, true);
	$store=$obj['store'];	
	$store_url=$obj['store_url'];	
	$ws_token=$obj['ws_token'];	
	$ws_api=$obj['ws_api'];	
	$ws_secret=$obj['ws_secret'];		
	...
	$result=$woocommerce->put('products/'.$_GET['id'].'/variations/'.$_GET['idsku'].'', $data);
...
$output = array("stock"=>$stock,"found"=>$found,"resultado"=>$result);
