...
if(!isset($total_paid_tax_incl) || $total_paid_tax_incl==""){
...
$mdata=array();
foreach($meta_data as $k2=>$v2){
	$mdata[$v2['key']]=$v2['value'];
}

$nif=$mdata['_billing_nif'];
  
//erpLog("Encomenda# ".$id." NIF: ".$nif." Nome:".$b_firstname." ".$b_lastname);  
//die();   
...
if($rDocExiste==0){
	erpLog("Doc não existe, NIF=$nif, CF=$bitConsumidorFinal  Mdata=".serialize($meta_data)."");  
#################################################################	ADICIONA CLIENTE  ######################################################		
...
###########################################################   /CRIA ENCOMENDA   ######################################################	
} else { 	erpLog("Doc já existe, NIF=$nif, CF=$bitConsumidorFinal  Mdata=".serialize($meta_data)."");  }
$mensagemHtml="";  
if($errormsg!=""){
$htmlmsg=$errormsg;	
$mensagem="0";	
