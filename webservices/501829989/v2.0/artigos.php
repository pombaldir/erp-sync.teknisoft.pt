<?php header('Access-Control-Allow-Origin: *');  header('Access-Control-Allow-Headers: : x-requested-with');	header('Access-Control-Allow-Methods: GET, POST, PUT'); 	header('Content-type: application/json; charset=utf-8'); 
include("index.php");
use Medoo\Medoo;
...
$fltPesoLiquido=stripslashes($_POST['fltPesoLiquido']);
...
$dataUpdt = $database->update("Tbl_Gce_Artigos", ["strDescricaoCompl" => $descr,"bitInactivo" => $inativo,"fltPesoLiquido" => $fltPesoLiquido], ["Id" => $idartigo]);
}
...
$tbla="Tbl_Gce_Artigos";    
if(isset($_POST['id_ref']) && $_POST['id_ref']==$idartigo){
    $tbla="Tbl_Gce_Referencias"; 
}     
...
if($tipo=="mainfoto"){
  ...
  $dataUpdt = $database->update($tbla, ["imgImagem" => Medoo::raw('CONVERT(varbinary(MAX), '.$imgContent.')')], ["Id" => $idartigo]);
...
$isReferencia=0;    
    ...
    if(isset($_POST['id_ref']) && $_POST['id_ref']==$id_artigo){
        $isReferencia=1;
    } 
	...
	$dataUpdt = $database->update("Tbl_Gce_Referencias", ["bitPortalWeb" => 1], ["Id" => $id_artigo]);
    }
	...
	$post = $database->select("Tbl_Gce_Referencias", [
	"strCodigo",
	"strCodCategoria",
	"strDescricao",
	"fltPesoLiquido",
	"strDescricaoCompl",
	"strObs",
	"strExcerto(strCodBarras)",
	"QuantStock"=>Medoo::raw('1'),
	"imgImagem"
	], [
		"Id" => $id_artigo,
		"LIMIT" => 1
    ...
    $qPrecos = getPrecosRef($post[0]['strCodigo']);   
    }    
	...
	# # FAMILIA
    if($isReferencia==0){    
    $tblFamilias="Tbl_Gce_ArtigosFamilias";   
    $fieldFam="strcodArtigo";    
    } else {
    $tblFamilias="Tbl_Gce_ReferenciasFamilias";  
    $fieldFam="strcodReferencia";    
    }
	$qFamilias = $database->select($tblFamilias, [
    ...
    //$strCodFamilia=$arrfam[1]['codigo'];    
    $strCodFamilia="";   
    $strCodigo=$post[0]['strCodigo'];    
    $strObs=$post[0]['strObs']; 
    $strDescricao=$post[0]['strDescricao'];
    $strDescricaoCompl=$post[0]['strDescricaoCompl'];
	$fltPesoLiquido=$post[0]['fltPesoLiquido'];
    $strCodCategoria=$post[0]['strCodCategoria'];
    $strCodBarras=$post[0]['strCodBarras'];
     
        
    # # REFERENCIAS
    $arrReferencias=$rowRefer=array();   
    if($isReferencia==0){     
	$qReferencias = $database->select("Tbl_Gce_ArtigosReferencias", [
	"[>]Tbl_Gce_Artigos" => ["strCodArtigo" => "strCodigo"],
    "[>]Tbl_Gce_ArtigosPrecos" => ["Tbl_Gce_Artigos.strCodigo" => "strCodArtigo"],    
	],[
    "Tbl_Gce_ArtigosReferencias.strCodReferencia",    
	"Tbl_Gce_ArtigosReferencias.strCodGrelhaHor",
	"Tbl_Gce_ArtigosReferencias.strCodGrelhaHorLin",
	"Tbl_Gce_ArtigosReferencias.strCodGrelhaVer",
    "Tbl_Gce_ArtigosReferencias.strCodGrelhaVerLin",
    "fltPreco",    
	], [
		"Tbl_Gce_Artigos.Id" => $id_artigo
	]);	
	...
	$qReferencias = $database->select("Tbl_Gce_ArtigosReferencias", [
    "[>]Tbl_Gce_ReferenciasPrecos" => ["Tbl_Gce_ArtigosReferencias.strCodReferencia" => "strCodReferencia"],    
	],[
    "Tbl_Gce_ArtigosReferencias.strCodReferencia",    
	"Tbl_Gce_ArtigosReferencias.strCodGrelhaHor",
	"Tbl_Gce_ArtigosReferencias.strCodGrelhaHorLin",
    ...
    "fltPreco",    
		...
		"Tbl_Gce_ArtigosReferencias.strCodReferencia" => $post[0]['strCodigo']
	]);	
    ...
    if(sizeof($qReferencias)>0){    
    $arrReferencias=$qReferencias[0]; 
    ## Grelha Vertical    
    $queryGV=$database->select("Tbl_Gce_Grelhas_Lin",["strDescricao"], ["strCodGrelha" => $arrReferencias['strCodGrelhaVer'],"strCodigo" => $arrReferencias['strCodGrelhaVerLin']]);  
    $rowRefer['var'][$arrReferencias['strCodGrelhaVer']] = $queryGV[0]['strDescricao'];        
    ## Grelha Horizontal   
    $queryGH=$database->select("Tbl_Gce_Grelhas_Lin",["strDescricao"], ["strCodGrelha" => $arrReferencias['strCodGrelhaHor'],"strCodigo" => $arrReferencias['strCodGrelhaHorLin']]);  
    $rowRefer['var'][$arrReferencias['strCodGrelhaHor']] = $queryGH[0]['strDescricao'];     
    $rowRefer['strCodReferencia'] = $arrReferencias['strCodReferencia'];   
    $rowRefer['fltPreco'] = $arrReferencias['fltPreco'];      
    } 
    
            
	//print_r($database->error()); 
	$detalhes=array("strCodigo"=>$strCodigo,"strCodCategoria"=>$strCodCategoria,"strCodBarras"=>$strCodBarras,"strDescricao"=>$strDescricao,"strDescricaoCompl"=>$strDescricaoCompl,"fltPesoLiquido"=>$fltPesoLiquido,"strObs"=>$strObs,"strCodFamilia"=>$strCodFamilia,"hasImage"=>$hasImage,"imgData"=>$imgData,"stock"=>$stock,"precos"=>$precos,"familias"=>$arrfam,"referencias"=>$rowRefer); 
...
if(isset($_POST['act_p']) && $act_pst=="uncheckweb" && ($_SERVER['REQUEST_METHOD'] === 'POST')){
	...
	$dataUpdt = $database->update("Tbl_Gce_Referencias", ["bitPortalWeb" => 0], ["Id" => $id_artigo]);
    } 
	...
	$output = array("success" => "$sucesso", "type" => "info", "message" => "$html_msg", "idartigo" => "$id_artigo", "log" => $database->log());

}
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
if(isset($_POST['act_p']) && $act_pst=="uncheckwebSerial" && ($_SERVER['REQUEST_METHOD'] === 'POST')){
	$art=$_POST['art']; 
	$dataUpdt = $database->update("Tbl_Gce_Artigos", ["bitPortalWeb" => 0], ["Id" => $art]);
	if($dataUpdt==1){
	   $sucesso=1; $html_msg="Artigos atualizados no ERP";   
	} else {
	   $sucesso=0; $html_msg="ERRO SQL ".$database->log();   	 
	}
	$output = array("success" => $sucesso, "type" => "info", "message" => $html_msg);
...
if(isset($_GET['act_g']) && $act_get=="view" && ($_SERVER['REQUEST_METHOD'] === 'GET')){
	...
	"strObs",
	"QuantStock",
	"fltPesoLiquido" => Medoo::raw("CAST(fltPesoLiquido AS DECIMAL(8,2))"),
	"bitInactivo",
	"fltPCReferencia",
	"imgImagem"
	], [
...
if(isset($_GET['act_g']) && $act_get=="sku" && ($_SERVER['REQUEST_METHOD'] === 'GET')){
	...
	$codartigo=$_GET['num'];
	$priceline=isset($_GET['lp']) ? filter_input(INPUT_GET, 'lp', FILTER_SANITIZE_NUMBER_INT) : 1; 
	
	$Tbl_Gce=$database->select("Gce_stk_real", ["QuantStock" => Medoo::raw('QuantStock-ReservaStock') ], ["strCodArtigo" => $codartigo]);	
...
$output = array("stock"=>$stock,"price"=>$price,"found"=>sizeof($Tbl_Gce));
}  	
...
if(isset($_GET['act_g']) && $act_get=="list" && ($_SERVER['REQUEST_METHOD'] === 'GET')){
    ...
    */
	...
	if(isset($tipoArtigo) && $tipoArtigo!="" && is_array($tipoArtigo)){
		if($sWhere==""){ $sWhere .= " WHERE "; } else { $sWhere .= " AND "; } 
		$sWhere .= "Tbl_Gce_Artigos.strTpArtigo IN (". implode(',', $tipoArtigo) .")";  
	}
