...
function updtArtigoLocal($erp,$store=NULL,$tpo,$id_ref=NULL){ 
global $mysqli;
if($tpo=="cb_artigo_a" && $store!=""){
$query = $mysqli->query("INSERT INTO artigos (id_erp,id_store,id_erp_ref) VALUES ('$erp','$store',$id_ref)") or die("updtArtigoLocal a) ". $mysqli->errno .' - '. $mysqli->error);
}
if($tpo=="cb_artigo_d"){
$idPartigoStore=getArtigoStoreId($erp);	
$query = $mysqli->query("delete from artigos where id_erp='$erp'") or die("updtArtigoLocal b) ".$mysqli->errno .' - '. $mysqli->error);
}

if($tpo=="cb_artigo_u" && $store!=""){  
$query = $mysqli->query("select idnum from artigos where  id_erp='$erp' and id_store='$store'") or die("updtArtigoLocal c) ".$mysqli->errno .' - '. $mysqli->error);
if($query->num_rows==0){
	updtArtigoLocal($erp,$store,"cb_artigo_a",$id_ref);	
}
}
...
$query = $mysqli->query("select id_store from artigos where id_erp='$erpID'") or die("getArtigoStoreId ".$mysqli->errno .' - '. $mysqli->error);
$dados = $query->fetch_array();
...
$query = $mysqli->query("select id_store from familias where id_erp='$erpID'") or die("getcategoriaStoreId ".$mysqli->errno .' - '. $mysqli->error);
$dados = $query->fetch_array();
return $dados['id_store'];
}
...
global $mysqli;
$query = $mysqli->query("select id_erp from familias where id_store='$storeFamId'") or die("getFamiliaERPId ". $mysqli->errno .' - '. $mysqli->error);
$dados = $query->fetch_array();
return $dados['id_erp'];
}
...
function ReferenciaExiste($idref){ 
global $mysqli;
$query = $mysqli->query("select COUNT(idnum) as total from artigos WHERE id_erp_ref='$idref'") or die($mysqli->errno .' - '. $mysqli->error);
$dados = $query->fetch_array();
return $dados['total'];
}

function getRefArtigoId($idref){ 
global $mysqli;
$query = $mysqli->query("select id_store from artigos where id_erp_ref='$idref' ORDER by idnum ASC LIMIT 0,1") or die("getRefArtigoId ".$mysqli->errno .' - '. $mysqli->error);
$dados = $query->fetch_array();
return $dados['id_store'];
}

function getRefArtigoERP($idartigo){ 
global $mysqli;
$query = $mysqli->query("select id_store from artigos where id_erp='$idartigo' ORDER by idnum ASC LIMIT 0,1") or die($mysqli->errno .' - '. $mysqli->error);
$dados = $query->fetch_array();
return $dados['id_store'];
}

function getRefArtigoERPprinc($idartigo){ 
    global $mysqli;
    $query = $mysqli->query("select id_store from artigos where id_erp='$idartigo' and id_erp_ref='$idartigo' ORDER by idnum ASC LIMIT 0,1") or die($mysqli->errno .' - '. $mysqli->error);
    $dados = $query->fetch_array();
    return $dados['id_store'];
}
 
function getAllArtigos(){ 
    global $mysqli; 
    $arr=array(); 
    $query = $mysqli->query("select id_store,id_erp,id_erp_ref from artigos where (id_erp<>id_erp_ref OR id_erp_ref IS NULL)  ORDER by idnum ASC") or die($mysqli->errno .' - '. $mysqli->error);
    while($dados = $query->fetch_assoc()){
        if($dados['id_erp_ref']==""){
            $refX=""; 
        } else {  
            $refX=getRefArtigoERPprinc($dados['id_erp_ref']); 
        }
        $arr[]=array("id"=>$dados['id_store'],"sku"=>$dados['id_erp'],"idArtRef"=>$refX);  
    }
    return $arr;
}
...
function ERP_Uncheck_Serial($array,$ref=NULL){
    global $mysqli;
    $prefs=settings_val(1);  

    $url = $prefs['erp_ws'].'/artigos.php';
    $fields = array(
                'art' => $array,
                'act_p' => "uncheckwebSerial",
                'auth_userid'=> $prefs['ws_token']
            );
    ...
    if($sucesso=="1" && $ref!=""){
        $mysqli->query("delete from artigos where id_erp_ref='$ref'") or die(json_encode($mysqli->errno .' - '. $mysqli->error));
    } 
    return $result;
}
